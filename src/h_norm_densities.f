c-----------------------------------------------------------------------
c At t=t_n, calculate square root of the integrand of the H_AdS^(0,sp), H_AdS^(1,sp), H_AdS^(2,sp) norms (defined in arxiv:1110.6794v2)
...
c The variable f1 represents the evoluton variable \barphi, while the variable f represents the full scalar field phi=(1-rho^2)^2*\barphi
c-----------------------------------------------------------------------


c-----------------------------------------------------------------------
c Compute H_AdS^(0,sp) of scalar field f=(1-rho^2)^2*f1
c-----------------------------------------------------------------------
...
!        h0spnormdensity_f0=1/abs(log(ct+dt))
...
!Take square root, so we can use DV calculation of L2-norm, and then square it, to compute the H_AdS(1,s) norm.
     ...
     &             sqrt(h0spnormdensity_f0)
                ...
                !hence we set sqrth0spnormdensity_f(i,j,k) to 0 at these points
...
c-----------------------------------------------------------------------
c Compute H_AdS^(1,sp) of scalar field f=(1-rho^2)^2*f1
c-----------------------------------------------------------------------
...
!        !e.g. dxcar_dxqssph(3,2)=dy/drho=sin(theta0)*cos(phi0)
     ...
     &              detg0_kerrads_qssph0)
        ...
        call derf_dxsph(f1,f1,f1,f1_t,f1_rho,f1_theta,f1_phi,
...
!compute second covariant derivative of f for surfaces of constant t,rho
     ...
     &                 *(1-rho0**2)**2*df1_dxqssph(c)
        ...
        call der2f_dxsphdxsph(f1,f1,f1,
     ...
     &        +2*(1-rho0**4)*f1_rho)/(1-rho0**2)
            d2f_dRaddRad=
     &       4*(1+rho0)**2*
     &      (4*(-1-4*rho0**2+rho0**4)*f10
     &       -(1-rho0**2)*(2*rho0*(1+3*rho0**2)*f1_rho
     &        -(1-rho0**4)*f1_rhorho))*(1/(1-rho0**2)**4)
            d2f_dRaddtheta=
     &       4*(1+rho0)**2*
     &      (4*(-1-4*rho0**2+rho0**4)*f1_theta
     &       -(1-rho0**2)*(2*rho0*(1+3*rho0**2)*f1_rhotheta
     &        -(1-rho0**4)*f1_rhorho))*(1/(1-rho0**2)**4)
            d2f_dRaddphi=
     &       4*(1+rho0)**2*
     &      (4*(-1-4*rho0**2+rho0**4)*f1_phi
     &       -(1-rho0**2)*(2*rho0*(1+3*rho0**2)*f1_rhophi
     &        -(1-rho0**4)*f1_rhorho))*(1/(1-rho0**2)**4)
